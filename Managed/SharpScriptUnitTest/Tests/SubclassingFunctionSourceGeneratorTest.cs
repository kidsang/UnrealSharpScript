...
namespace SharpScriptUnitTest.Types
{
	...
	public partial class USsTestGenFunctionSourceGenerator
	{
		...
		public unsafe ESsTestGenEnumSourceGenerator CallFuncEnum(ESsTestGenEnumSourceGenerator InValue, out ESsTestGenEnumSourceGenerator OutValue)
		{
			...
			EnumMarshaller<ESsTestGenEnumSourceGenerator>.ToNative(_params.Buffer + FuncEnum_InValue_Offset, InValue);
			InvokeFunctionCall(FuncEnum_NativeFunc, _params.Buffer);
			OutValue = EnumMarshaller<ESsTestGenEnumSourceGenerator>.FromNative(_params.Buffer + FuncEnum_OutValue_Offset);
			return EnumMarshaller<ESsTestGenEnumSourceGenerator>.FromNative(_params.Buffer + FuncEnum_ReturnValue_Offset);
		}
...
{
	/// <summary>
	/// Source-generator counterpart of <see cref="SubclassingFunctionManualTest"/>: exercises the same
	/// UE → C# UFunction round-trips, but on <see cref="USsTestGenFunctionSourceGenerator"/> whose
	/// subclassing bindings (function defs, native dispatch stubs, offsets) are produced by the
	/// SharpScript source generator rather than hand-written. Asserting identical behaviour proves the
	/// generator output is equivalent to the hand-written reference.
	/// </summary>
	[RecordFilePath]
	public class SubclassingFunctionSourceGeneratorTest : IUnitTestInterface
	{
		public bool RunTest()
		{
			USsTestGenFunctionSourceGenerator TestObj = NewObject<USsTestGenFunctionSourceGenerator>();
			...
			ESsTestGenEnumSourceGenerator inputEnum = ESsTestGenEnumSourceGenerator.Two;
			...
			UObject inputObject = NewObject<USsTestGenFunctionSourceGenerator>();
			...
			UClass inputClass = USsTestGenFunctionSourceGenerator.StaticClass!;
			...
			Utils.Assert(USsTestGenFunctionSourceGenerator.CallFuncStaticInt32(inputStaticInt32, out var outputStaticInt32) == inputStaticInt32);
			Utils.Assert(outputStaticInt32 == inputStaticInt32);

			string inputStaticString = "static-789";
			Utils.Assert(USsTestGenFunctionSourceGenerator.CallFuncStaticString(inputStaticString, out var outputStaticString) == inputStaticString);
