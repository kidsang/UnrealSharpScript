...
public static class ClassUtilities
{
	...
	public static List<UhtClass> GetInterfaces(this UhtClass classObj)
	{
		...
		{
			UhtEngineType engineType = baseStruct.EngineType;
			if (engineType is UhtEngineType.NativeInterface)
			{
				UhtClass? interfaceClass = baseStruct as UhtClass;
				interfaces.Add((UhtClass)interfaceClass!.AlternateObject!);
			}
		}
	...
	public static List<UhtClass> GetInterfacesIncludeSuper(this UhtClass classObj)
	{
		List<UhtClass> interfaces = new();
		foreach (UhtStruct baseStruct in classObj.Bases)
		{
			...
			if (engineType is UhtEngineType.NativeInterface)
			{
				UhtClass? interfaceClass = (UhtClass)baseStruct;
				while (interfaceClass != null)
				{
					UhtClass althernateClass = (UhtClass)interfaceClass.AlternateObject!;
					if (!interfaces.Contains(althernateClass))
					{
						interfaces.Add(althernateClass);
					}

					interfaceClass = interfaceClass.SuperClass;
				}
			}
		}
